...
// @license             BSD
// @develop             @es6  @chrome85  @TypeScript @eagle:2.0 Build40  @cmd:  tsc 微博收集助手.ts -w '--downlevelIteration' --target ES6 
// @author              黄逗酱酱
// @date                11/28/2021
// @note                @0.1 JS编写  兼容旧版的脚本诞生 但是并未深度获取九宫格以外和视频
// @note                @0.2 ts重写  支持新版支持所有图片视频加入旧版兼容特性(并未支持)
// @note                @0.3 优化    通过多次定时执行以获得更快的按钮添加速度 并解决部分小问题 减少@ts-ignore便签使用方便后期更好维护
// ==/UserScript==
// TS自动编译的向下兼容异步函数
...
(function (unsafeWindow: Window, Store, $: Function, $$: Function, Sleep = ToEagle._Sleep) {
    ...
    function FunMian(): void {
       ...
       `,         /**
        ...
        }; if (!$IS_funStart.HM__IF()) { debugger; return; };// 没有需要采集的界面则或者用户未同意使用协议则退出
        /***********************会被多数函数调用的模块*************************** */
        /**
         * Call类型
         * @param a 
    ...
    ; (async function (): Promise<void> {
        Fun_toPromise(FunMian)
            ...
            .catch(async function (/*失败第一次*/) {
                await Sleep(1500);
                return Fun_toPromise(FunMian);
            })
            // 失败了5秒后重试
            .catch(async function (/*失败第二次*/) {
                await Sleep(5000);
                return Fun_toPromise(FunMian);
            })
            .catch(async function (/*放弃治疗*/) {
                console.debug("脚本错误", this)
                console.debug("错误的函数", FunMian)
                console.debug("错误次数", 3)
            })
    })();
